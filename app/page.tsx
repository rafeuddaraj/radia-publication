import { Footer } from "@/components/footer";
import { Navbar } from "@/components/navbar";
import { Toaster } from "sonner";

export default async function Home() {
  // console.log(await auth());

  return (
    <>
      <Navbar />
      <main className="sm:container mx-auto w-[90vw] h-auto scroll-smooth">
        <Toaster expand={true} position="bottom-right" duration={2000} />
        <div className="flex sm:min-h-[85.5vh] min-h-[85vh] flex-col items-center justify-center text-center px-2 sm:py-8 py-12">
          <h1 className="text-3xl font-bold mb-4 sm:text-6xl">
            স্বাগতম নেক্সট পাবলিকেশন
          </h1>
          <p className="mb-8 sm:text-lg max-w-[800px] text-muted-foreground">
            স্বাগতম নেক্সট পাবলিকেশন হলো একটি অত্যাধুনিক ভার্চুয়াল বইয়ের
            অ্যাপ্লিকেশন। এটি বই পড়ার অভিজ্ঞতাকে আরও সহজ, আকর্ষণীয় এবং আধুনিক
            করে তুলতে তৈরি। এখানে আপনি পাবেন হাজারো ই-বুক, অডিওবুক, এবং
            ইন্টারেক্টিভ কন্টেন্ট যা আপনার পড়ার অভ্যাসকে এক নতুন মাত্রায় নিয়ে
            যাবে।
          </p>
        </div>
      </main>
      <Footer />
    </>
  );
}
